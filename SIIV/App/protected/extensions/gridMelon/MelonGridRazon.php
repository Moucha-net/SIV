...
class MelonGridRazon extends CWidget
{
	public $modelo=null;
										...
										$this->count=$this->count+1;
											...
											{	echo "<tr>";
												...
												foreach ($this->colContenido as $colC) {
													...
													{
														if($colC == "idTipoRechazo")
														{
															$tipRech = (TipoRechazo::model()->exists('idtipoRechazo='.$puesto->$colC)) ? TipoRechazo::model()->find('idtipoRechazo='.$puesto->$colC)->nombreTipoRechazo : "X".$clave;
															echo "<td>".$tipRech."</td>";
														}
														else
														{
